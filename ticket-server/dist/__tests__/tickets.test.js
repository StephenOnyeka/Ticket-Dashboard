...
beforeAll(async () => {
    // Obtain agent token
    const agentRes = await (0, supertest_1.default)(index_1.default).post('/api/auth/login').send({
        email: 'agent@support.com',
        password: 'agent123',
    });
    agentToken = agentRes.body.token;
    // Obtain guest token
    const guestRes = await (0, supertest_1.default)(index_1.default).post('/api/auth/login').send({
...
describe('GET /api/tickets', () => {
    it('returns a paginated list of tickets', async () => {
        const res = await (0, supertest_1.default)(index_1.default).get('/api/tickets');
        expect(res.status).toBe(200);
    ...
    it('filters tickets by status=open', async () => {
        const res = await (0, supertest_1.default)(index_1.default).get('/api/tickets?status=open');
        expect(res.status).toBe(200);
        expect(res.body.data.every((t) => t.status === 'open')).toBe(true);
    });
    it('filters tickets by channel=email', async () => {
        const res = await (0, supertest_1.default)(index_1.default).get('/api/tickets?channel=email');
        expect(res.status).toBe(200);
        expect(res.body.data.every((t) => t.channel === 'email')).toBe(true);
    });
    it('searches tickets by keyword', async () => {
        const res = await (0, supertest_1.default)(index_1.default).get('/api/tickets?search=billing');
        expect(res.status).toBe(200);
        expect(res.body.data.length).toBeGreaterThan(0);
    });
    it('respects pagination params', async () => {
        const res = await (0, supertest_1.default)(index_1.default).get('/api/tickets?page=1&limit=5');
        expect(res.status).toBe(200);
...
describe('GET /api/tickets/:id', () => {
    it('returns a single ticket by id', async () => {
        const res = await (0, supertest_1.default)(index_1.default).get('/api/tickets/ticket-001');
        expect(res.status).toBe(200);
        expect(res.body.id).toBe('ticket-001');
        expect(res.body).toHaveProperty('comments');
    });
    it('returns 404 for unknown ticket', async () => {
        const res = await (0, supertest_1.default)(index_1.default).get('/api/tickets/ticket-999');
...
describe('POST /api/tickets', () => {
    ...
    it('rejects ticket creation without auth', async () => {
        const res = await (0, supertest_1.default)(index_1.default).post('/api/tickets').send({
